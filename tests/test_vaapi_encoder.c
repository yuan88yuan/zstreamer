/*=============================================================================
    test_vaapi_encoder.c — VA-API encoder property and smoke tests
=============================================================================*/
...
static void test_factory_and_properties(void)
{
    ...
    assert(desc != NULL);
    assert(desc->nb_properties >= 8);
    assert(desc->nb_pads == 3);

    const zst_element_desc_t* alias = zst_element_factory_get_desc(ZST_VAAPI_VIDEO_ENCODER_FACTORY_ALIAS);
    assert(alias != NULL);

    zst_element_t* enc = zst_element_factory_make(ZST_VAAPI_VIDEO_ENCODER_FACTORY_ALIAS);
    assert(enc != NULL);
    assert(enc->ops != NULL);
    assert(strcmp(enc->ops->name, ZST_VAAPI_VIDEO_ENCODER_FACTORY) == 0);

    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_CODEC, "h265") == ZST_OK);
    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_BITRATE, "2500000") == ZST_OK);
    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_GOP_SIZE, "60") == ZST_OK);
    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_PRESET, "speed") == ZST_OK);
    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_RATE_CONTROL, "VBR") == ZST_OK);
    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_FPS, "30/1") == ZST_OK);

    char value[64];
    assert(zst_element_get_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_CODEC, value, sizeof(value)) == ZST_OK);
    assert(strcmp(value, "h265") == 0);
    assert(zst_element_get_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_BITRATE, value, sizeof(value)) == ZST_OK);
...
static void test_runtime_or_skip(void)
{
    ...
    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_BITRATE, "1000000") == ZST_OK);
    assert(zst_element_set_property(enc, ZST_VAAPI_VIDEO_ENCODER_PROP_GOP_SIZE, "15") == ZST_OK);

    zst_result_t ret = zst_element_set_state(enc, ZST_STATE_READY);
    if (ret != ZST_OK) {
        printf("SKIP: VA-API runtime or /dev/dri render node is not available in this environment\n");
        zst_element_destroy(enc);
        return;
    }
    ...
    for (uint64_t i = 0; i < 30; i++) {
        zst_buffer_t* in = make_i420_frame(128, 128, i);
        assert(in != NULL);
        zst_buffer_t* out = NULL;
        ret = enc->ops->process(enc, in, &out);
        zst_buffer_unref(in);
        if (ret != ZST_OK) {
            printf("SKIP: VA-API encoder init/encode is not supported by this driver (ret=%d frame=%" PRIu64 ")\n", ret, i);
