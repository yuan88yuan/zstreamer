...
static const char*
aacdec_sample_fmt_to_string(int fmt)
{
...
static void
aacdec_buf_free(zst_buffer_t* buf)
{
    if (!buf) return;

    aacdec_buf_priv_t* priv = buf->metadata;
    if (priv) {
...
static zst_result_t
aacdec_emit_buffer(zst_element_t* el, zst_buffer_t* buf, zst_buffer_t** out)
{
...
static zst_result_t
aacdec_emit_frame(zst_element_t* el, aac_decoder_t* s, zst_buffer_t** out)
{
    ...
    aacdec_buf_priv_t* buf_priv = abuf->metadata;
    if (!buf_priv) {
    ...
    return aacdec_emit_buffer(el, abuf, out);
}
...
aacdec_process(zst_element_t* el, zst_buffer_t* in, zst_buffer_t** out)
{
    ...
    while (1) {
        ...
        zst_result_t emit_ret = aacdec_emit_frame(el, s, out);
    ...
    if (in->flags & ZST_BUFFER_FLAG_EOS) {
        ...
        return aacdec_emit_buffer(el, eos_buf, out);
    }
...
static zst_result_t
aacdec_sink_push(zst_pad_t* pad, zst_buffer_t* buf)
{
    if (!pad || !pad->parent || !buf) return ZST_ERROR;
    zst_buffer_t* out = NULL;
    zst_result_t ret = aacdec_process(pad->parent, buf, &out);
    if (out) {
...
static zst_caps_t*
aacdec_get_caps(zst_element_t* el, zst_pad_t* pad, const zst_caps_t* filter)
{
    (void)filter;
    aac_decoder_t* s = el->priv;
    ...
    if (pad == s->sinkpad) {
        ...
        zst_caps_append(caps, zst_caps_struct_create_audio("audio/x-raw",
                                                           s->channels,
                                                           s->sample_rate,
                                                           aacdec_sample_fmt_to_string(s->format)));
    }
...
zst_aac_decoder_create(void)
{
    ...
    priv->sinkpad->push = aacdec_sink_push;
